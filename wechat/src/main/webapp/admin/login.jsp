...
%><%
    ...
    if (user != null) {
        ...
        GeliUtils.getDao().create(rSession);
        Cookie cookie = new Cookie("geli-session", rSession.getSessionId());
        cookie.setMaxAge(60 * 60 * 3);//有效期3个小时
...
#back
{
    ...
    <body onload='document.getElementById("return").value = location.href.replace(/http:\/\/(.*)\/(.*)/, "http://$1/login.jsp");'>
        <form id="login" action="${authUri}" method="post"> 
            ...
            <span style="font-size: 40px; padding: 10px; color: #666;">今日聚超值</span>
            </div>
            <fieldset id="inputs"> 
